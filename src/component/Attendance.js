...
function Attendance() {
  ...
  const attendanceDetails = () => {
    axios
      .post(
        "http://localhost:8080/attendance/details",
        {
          email: atob(
            unescape(
      ...
      .then(function (response) {
        // handle success
        console.log(
          "response for check btn : ",
          response.data.allowedToCheckIn
        );
  ...
  return (
    <div>
      <span className="attendance__total__time">{totalHours} Hrs</span>
      {attendanceBtnFlag ? (
        <button
          ...
          className={
            "attendance__btn " +
            (attendanceButtonText ? "" : "attendance__check__out")
          }
        >
